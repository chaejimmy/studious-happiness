...
extension Font.Weight{
    /// Represents the available font weights for Circular font, which contains 4 typefaces the rest are for the fillup purpose.
...
extension Font{
    ...
    static func circular(size: CGFloat, weight: Font.Weight.Circular = .book) -> Font{
        return Font.custom(weight.name, size: size)
    }
